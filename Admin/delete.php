...
<div id="conten" class="col-lg-10 col-md-9 col-sm-9 col-xs-12 container-fluid">
    <div class="panel panel-default main">
      <div class="panel-heading">Xóa bài viết</div>
      <div class="panel-body">
      <?php if(isset($_POST['listdelete'])){?>
      
      <div class="alert alert-danger">
 <h4> <strong>Bạn chắc chắn muốn xóa các bài viết này?</strong></h4>
</div>
      <?php 
      	$list = $_POST['listdelete'];
      	$_SESSION['listdelete'] = $list;
      	foreach ($list as $item => $item_value  ){
      		//echo $item_value;
      		$sql = "SELECT * FROM post WHERE Id =".$item_value;
	...
	echo '<div class="alert alert-warning" role="alert">Chưa chọn bài viết để xóa</div>';
      ...
      <a href="list.php"><button class="btn btn-primary" type="button" name="btnno">Cancel</button></a>
      </form>
      </div>
      
      <?php 
      	if(isset($_POST['btnyes'])){
      		$list = $_SESSION['listdelete'];
      		$_SESSION['listdelete']=null;
      		//echo "Da bam nut ok";
      		foreach ($list as $item => $item_value  ){
      			$sqlDelete="DELETE FROM post WHERE Id = ".$item_value;
